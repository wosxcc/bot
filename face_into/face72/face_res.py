...
def face_net(batch_size,height, width, n_classes,learning_rate):
    ...
    with tf.variable_scope('conv7') as scope:
        ...
        conv7 = tf.nn.conv2d(relu66, W7, strides=[1, 1, 1, 1], padding='SAME')
    ...
    with tf.variable_scope("fc1") as scope:
        ...
        weights1 =weight_variable([dim, 292])
        biases1 = bias_variable([292])
        fc1 = tf.nn.relu(tf.matmul(reshape, weights1) + biases1, name="fc1")

    with tf.variable_scope("output") as scope:
        weights2 = weight_variable([292, n_classes])
    ...
    for step in np.arange(MAX_STEP):
        for i in range(BATCH_SIZE):
            ...
            _ ,tra_loss= sess.run([graph['optimize'],graph['cost']],feed_dict={
                        ...
                        graph['y']: np.reshape(Y_data[xb], (1, 146))})
