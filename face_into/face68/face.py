...
def read_text(txt_name):
    ...
    try:
        for i in range(len(other_result)):
            ...
            image_payh.append('E:/face68/trainb/'+data_file[0])
            position.append(data_file[1:])
    except:
        print('Erro')
...
def get_batch(image, position, img_W, img_H, batch_size, capacity):
    ...
    position_batch = tf.reshape(position_batch, [batch_size, 112])
...
def val(test_file):
    ...
    with tf.Graph().as_default():
        ...
        op_intp = np.zeros(112, np.float32)
        p, r = inference(image, op_intp, 1, 112)
        # print('看看p的值：',p)
...
for file in os.listdir(file_path):
    ...
    for i in range(int(len(prediction[0])/2)):
        cv.circle(img, (int(prediction[0][i*2] * img.shape[1]), int(prediction[0][i*2+1] * img.shape[0])), 2, (0, 255, 255), -1)
