...
def train_loss(logits,ydata):
    with tf.variable_scope('loss') as scope:
        tloss = tf.reduce_sum(tf.square(logits - ydata), name='loss')
    return tloss

def trian_op(tloss,learning_rate):
...
def run_training(txt_name):
    ...
    logs_train_dir = './face_key/silm_face/'
    X_data, Y_data = read_img(txt_name)
    image_batch = tf.placeholder(tf.float32, shape=[None, IMG_W, IMG_H, 3], name='input')
    image_label = tf.placeholder(tf.float32, shape=[None, N_CLASSES], name='labels')
    phase_train = tf.placeholder(tf.bool,name='phase_train')
    count_y, _= inference(image_batch, 0.8, phase_train,N_CLASSES,0.005)
    tloss =train_loss(count_y, image_label)
    train_op = trian_op(tloss,learning_rate)
    ...
    for step in np.arange(MAX_STEP):
        ...
        _, tra_loss , tra_y_conv = sess.run([train_op,tloss,count_y],feed_dict={
            image_batch: np.reshape(batch_img, (BATCH_SIZE, IMG_H, IMG_W, 3)),
            image_label: np.reshape(batch_lab, (BATCH_SIZE, N_CLASSES)),
            phase_train:True})
        ...
        for node in sess.graph.as_graph_def().node:
            ...
            if (node.name.startswith('InceptionResnet') or node.name.startswith('embeddings') or
                    node.name.startswith('input') or node.name.startswith('labels') or
                    node.name.startswith('phase_train') or node.name.startswith('Reshape_1')):
        ...
        if step % 50 == 0:
            ...
            #         if 'use_locking' in node.attr: del node.attr['use_locking']
            #
            # print('看看什么东西',sess.graph_def)
            constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                       ['InceptionResnetV1/Bottleneck/BatchNorm/Reshape_1'],variable_names_whitelist=whitelist_names)
...
with tf.Graph().as_default():
    ...
    tra_y_conv,_= inference(image_batch, 0.8, phase_train, N_CLASSES, 0.005)
    saver = tf.train.Saver()
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, log_device_placement=False)) as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            for file in os.listdir(file_path):
                ...
                prediction = sess.run(tra_y_conv, feed_dict={
                    image_batch: np.reshape(image, (1, IMG_H, IMG_W, 3)),
                    image_label: np.reshape(op_intp, (1, N_CLASSES)),
                    phase_train: True})
