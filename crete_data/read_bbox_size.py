...
windows_name='E:/2018-08-02/no person'   # 文件位置
...
def draw_circle(event,x,y,flags,param):
    ...
    if event==cv.EVENT_LBUTTONUP:
        ...
        if xwidth*yheight>=200:
            ...
            drawing=False
count_c=0
keep_num=int(len(os.listdir(path[0:-1]))/2)+11002
# keep_num=
boxss  = open( 'boxscss.txt', 'w')
            ...
            if k==ord('c'): # c转换类型
                ...
                if len(sum_init)>0:
                    ...
                    for bbox in sum_init:
                        boxss.write(str(bbox[1]/img.shape[1])+' '+str(bbox[2]/img.shape[0])+' '+str(bbox[3]/img.shape[1])+' '+str(bbox[4]/img.shape[0])+'\n')
