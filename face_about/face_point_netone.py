...
def face_net(batch_size,height, width, n_classes,n_nature,learning_rate,phase_train):
    ...
    with tf.variable_scope('botface'):
        ...
        with tf.variable_scope("output") as scope:
            weights2 = weight_variable([256, n_nature+n_classes])
            biases2 = bias_variable([n_nature+n_classes])
        ...
        with tf.variable_scope("lossand") as scope:
            ...
            cross_entropye = tf.nn.softmax_cross_entropy_with_logits(logits=tf.slice(y_conv, [0, 0], [batch_size, 3]),
                                                                           labels=tf.slice(y, [0, 0], [batch_size, 3]), name="xentropy_per_examplee")
            losse = tf.reduce_mean(cross_entropye, name="losse")

            cross_entropyg = tf.nn.softmax_cross_entropy_with_logits(logits=tf.slice(y_conv, [0, 3], [batch_size, 3]),
                                                                           labels=tf.slice(y, [0, 3], [batch_size, 3]), name="xentropy_per_exampleg")
            lossg = tf.reduce_mean(cross_entropyg, name="lossg")

            cross_entropysex = tf.nn.softmax_cross_entropy_with_logits(logits=tf.slice(y_conv, [0, 6], [batch_size, 3]),
                                                                           labels=tf.slice(y, [0, 6], [batch_size, 3]), name="xentropy_per_examplesex")
            losssex = tf.reduce_mean(cross_entropysex, name="losssex")


            loss1 = tf.reduce_sum(tf.square(tf.slice(y, [0, 9], [batch_size, n_classes]) - tf.slice(y_conv, [0, 9], [batch_size, n_classes])), name='loss2')
...
def run_training():
    ...
    read_train_dir = './face_point/1204/'
    seave_train_dir = './face_point/1204/'
    ...
    for step in np.arange(MAX_STEP):
        ...
        print('得到的值',tra_y_conv[:5,:16])
        print('输入的值',input_y[:5,:16])
