...
def recognize(jpg_path, pb_file_path,img_w,img_h):
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            print (input_x)
            out_softmax = sess.graph.get_tensor_by_name("botface/output/output:0")
            print (out_softmax)
            ...
            for file in os.listdir(jpg_path):
                ...
                img_out_softmax = sess.run(out_softmax, feed_dict={input_x:np.reshape(ximg, [-1, img_w,img_h, 3]),  phase_train:False})

                print(img_out_softmax[0][0:16])
                ...
                for i in range(int(len(img_out_softmax[0]) / 2) - 6):
                    cv.circle(img, (int(img_out_softmax[0][12+i * 2] * img.shape[1]), int(img_out_softmax[0][12+i * 2 + 1] * img.shape[0])), 2,
                                     (0, 255, 255), -1)
...
recognize("E:/xbot/face_into/face68/image_test", "./face_point/1204/botface.pb",160,160)
