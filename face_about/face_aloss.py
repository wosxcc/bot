...
def face_net(batch_size,height, width, n_classes,learning_rate,phase_train):
    ...
    with tf.variable_scope('botface'):
        ...
        with tf.variable_scope("lossand") as scope:
            ...
            loss = tf.reduce_sum(tf.square(y - y_conv), name='loss')
...
def val_train(X_data,Y_data):
    ...
    for xb in range(BATCH_SIZE):
        ...
        for xxx in range(int((n_classes - 12) / 2)):
            nx = int((Y_data[xb][xxx * 2 + 2] + 1) / 2 * IMG_W)
            ny = int((Y_data[xb][xxx * 2 + 3] + 1) / 2 * IMG_W)
...
def run_training():
    ...
    read_train_dir = './face_point/1207/'
    seave_train_dir = './face_point/1207/'

    # lr = tf.constant(0.005, dtype=tf.float32, name="lrx")
    #lr =tf.placeholder(tf.float32, name='learnings')
    lr = tf.Variable(0.006, trainable=False)
    ...
    graph= face_net(BATCH_SIZE, IMG_H,IMG_W, n_classes,lr,phase_train)
    ...
    for step in np.arange(MAX_STEP):
        ...
        _, tra_loss , tra_y_conv ,input_y= sess.run([graph['optimize'],graph['loss'],graph['y_conv'],graph['y']],feed_dict={
                    ...
                    graph['y']: np.reshape(batch_y, (BATCH_SIZE, n_classes)),
                    phase_train:True})

        print('开始你的表演')
        # print(sess.run(tf.slice(tf.convert_to_tensor(np.reshape(batch_lab, (BATCH_SIZE, n_classes))), [0, 3], [BATCH_SIZE, 3])))
        # print(sess.run(tf.slice(tf.convert_to_tensor(np.reshape(batch_lab, (BATCH_SIZE, n_classes))), [0, 6], [BATCH_SIZE, 3])))
        # print(sess.run(tf.slice(tf.convert_to_tensor(np.reshape(batch_lab, (BATCH_SIZE, n_classes))), [0, 0], [BATCH_SIZE, 3])))
        print('得到的值',tra_y_conv[:2,:16])
        print('输入的值',input_y[:2,:16])
    ...
    with tf.Session() as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            for file in os.listdir(file_path):
                ...
                msex =prediction[0][:3].tolist()
                osex = '性别：不详'
                if msex.index(max(msex))==1:
                    osex = '性别：女'
                if msex.index(max(msex))==2:
                    osex = '性别：男'

                mxiao = prediction[0][3:6].tolist()
                oxiao = '表情：正常'
                if mxiao.index(max(mxiao)) == 1:
                    oxiao = '表情：微笑'
                if mxiao.index(max(mxiao)) == 2:
                    oxiao = '表情：大笑'

                mglass = prediction[0][6:9].tolist()
                oglass = '眼镜：没有'
                if mglass.index(max(mglass)) == 1:
                    oglass = '眼镜：有'
                if mglass.index(max(mglass)) == 2:
                    oglass = '眼镜：墨镜'

                oage= '年龄： 不详'
                if prediction[0][9]>0.5:
                    oage = '年龄：'+str(int(prediction[0][10]*100))

                odf = '得分 ：'+str(prediction[0][11])

                img = dram_chinese(img, odf, char_x=20, char_y=20, char_size=30, fillColor=(255, 255, 0))
                img = dram_chinese(img, osex, char_x=20, char_y=50, char_size=30, fillColor=(255, 255, 0))
                img = dram_chinese(img, oage, char_x=20, char_y=80, char_size=30, fillColor=(255, 255, 0))
                img = dram_chinese(img, oxiao, char_x=20, char_y=110, char_size=30, fillColor=(255, 255, 0))
                img = dram_chinese(img, oglass, char_x=20, char_y=140, char_size=30, fillColor=(255, 255, 0))
                ...
                for i in range(int(len(prediction[0]) / 2)-6):
                    cv.circle(img, (int(prediction[0][12+i * 2] * img.shape[1]), int(prediction[0][12+i * 2 + 1] * img.shape[0])), 2,
                              (0, 255, 255), -1)
                cv.imshow('img', img)
                cv.waitKey()
                cv.destroyAllWindows()
