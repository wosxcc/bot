...
def run_training():
    ...
    logs_train_dir = './faceID/deep_net/'
    X_data = get_img_data()
    ...
    for step in np.arange(MAX_STEP):
        ...
        for i in range(BATCH_SIZE):
            xb= (step%82)*32+i
            _, tra_loss, sess_pos, sess_neg = sess.run([graph['optimize'],graph['loss'],graph['d_pos'],graph['d_neg']],feed_dict={graph['x']: np.reshape(X_data[xb], (4, IMG_W, IMG_H, 3))})
            count_pos+=sess_pos
            count_neg+=sess_neg
            loss_avg+=tra_loss
