...
def face_net(batch_size,height, width, n_classes=512,learning_rate=0.001,margin=0.3,run_train=True):
    ...
    if run_train==True:
        ...
        negative_out1 =  tf.slice(y_conv, [2, 0], [1, n_classes])
        negative_out2 = tf.slice(y_conv, [3, 0], [1, n_classes])
        ...
        d_neg1 = tf.norm(anchor_out - negative_out1, axis=1)
        d_neg2 = tf.norm(anchor_out - negative_out2, axis=1)
        # neg = tf.concat([d_neg1, d_neg2],0)
        # print('我的',neg)
        # d_neg =tf.reduce_mean(neg)
        # # d_neg=tf.add(d_neg1,d_neg2)/2
        # print('你有错',d_neg)
        loss = tf.maximum( margin + d_pos - d_neg1,margin + d_pos - d_neg2)
