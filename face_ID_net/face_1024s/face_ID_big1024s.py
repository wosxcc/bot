...
def run_training(txt_name):
    ...
    read_train_dir = '../face72/face1024s/'
    saver_model_dir ='../face72/face1024s/'
    X_data = read_image()
    graph= face_net(BATCH_SIZE, IMG_H,IMG_W, N_CLASSES,learning_rate,margin=2,run_train=True)
    sess = tf.Session()
    ...
    for step in np.arange(MAX_STEP):
        ...
        if step % 50 == 0:
            ...
            constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                       ['output/output'])
            with tf.gfile.FastGFile(saver_model_dir + 'face72.pb', mode='wb') as f:
                f.write(constant_graph.SerializeToString())
            # 每迭代50次，打印出一次结果
        if step % 200 == 0 or (step + 1) == MAX_STEP:
            checkpoint_path = os.path.join(saver_model_dir, 'model.ckpt')
