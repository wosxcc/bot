...
from Model_learing.subpixel import SubPixelUpscaling
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
    ...
    Args:
        nb_classes: 类数
        img_input: 形状元组（通道、行、列）或（行、列、通道）
        include_top: 包含最终致密层的标志
        depth: 数量或层次
        nb_dense_block: ：添加到结束的密集块的数目（一般为3）
        growth_rate: 每个密集块添加的过滤器数量
        nb_filter: 过滤器的初始数量。默认值1表示过滤器的初始数目是2×增长率
        nb_layers_per_block: 每个密集块中的层数。
                                可以是-1，正整数或列表。
                                如果- 1，从网络的深度计算NbjLayel-PixBoad。
                                如果为正整数，则每密集块设置若干层。
                                如果是列表，则使用NBY层。注意列表大小必须
                                Be（NbxDeNeSyBug + 1）
        bottleneck: 添加瓶颈块
        reduction: 过渡块的还原因子。注意：还原值被反转以计算压缩。
        dropout_rate: 神经元使用率
        weight_decay: 权重衰减率
        subsample_initial_block: 设置为true，以对初始卷积进行子采样。在添加密集块之前添加MaxPool2D。
        subsample_initial:
        activation: 在顶层的激活类型。可以是“SuftMax”或“乙状结肠”。
注意，如果使用sigmoid ，则类必须为1。
    Returns:具有nb_layers 层的conv_block张量
    '''
