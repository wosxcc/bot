...
while(1):
    ...
    # print('fmeans',fmeans)
    centroids,variance=kmeans(fmeans,5)
    ...
    for x in range(sx):
        for y in range(sy):
            if codess[x][y]==0:
                nimg[y * steps:(y + 1) * steps, x * steps:(x + 1) * steps, 0] = int(np.mean(centroids[0]))
            elif codess[x][y]==1:
                nimg[y * steps:(y + 1) * steps, x * steps:(x + 1) * steps, 1] = int(np.mean(centroids[1]))
            elif codess[x][y] == 2:
                nimg[y * steps:(y + 1) * steps, x * steps:(x + 1) * steps, 2] = int(np.mean(centroids[2]))
            elif codess[x][y] == 3:
                nimg[y * steps:(y + 1) * steps, x * steps:(x + 1) * steps, :] = [255, 255, 0]
            elif codess[x][y] == 4:
                nimg[y * steps:(y + 1) * steps, x * steps:(x + 1) * steps, :] = [255, 0, 255]
            elif codess[x][y] == 5:
